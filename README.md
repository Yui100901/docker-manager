...
需要 Docker API 的命令默认管理本地 Docker daemon。也可以使用 Docker 标准环境变量、配置文件或全局参数指向远程 Docker:

```bash
DOCKER_HOST=tcp://192.168.31.57:2376 DOCKER_TLS_VERIFY=1 DOCKER_CERT_PATH=/root/.docker/certs dm health
dm --docker-host tcp://192.168.31.57:2376 --docker-tls-verify --docker-cert-path /root/.docker/certs health
```
...
docker_host: tcp://192.168.31.57:2376
...
Bash 补全依赖系统的 `bash-completion` 包；如果 `dm <Tab>` 不能展开候选，先确认当前 shell 已加载 bash-completion。Debian/Ubuntu 可执行 `sudo apt install bash-completion`，然后重新打开 shell，或手动 `source /usr/share/bash-completion/bash_completion`。
...
如需专门复测 shell completion，可运行:

```bash
bash scripts/completion-test.sh --dm-bin ./dm --keep-workdir
```

```powershell
.\scripts\completion-test.ps1 -DmBin .\dm.exe -KeepWorkDir
```

completion 深测脚本不会主动拉取外部镜像；有 Docker 时会复用本机已有镜像创建临时容器和 volume，用来验证容器、镜像和 volume 候选。没有 Docker 或没有本地 tagged 镜像时，对应资源候选用例会跳过。
...
检查范围包括 Docker daemon endpoint、权限和版本、dm 配置文件、代理环境变量和 `.dm.yaml` 代理格式、私有 CA 路径、Docker daemon registry 配置、输出目录剩余空间/剩余 inode/写入探测、Docker config、credential helper、registry `/v2/` 连通性和 Docker RegistryLogin 结果，以及 Go/vendor/e2e 测试前置条件。未指定 `--registry` 时会跳过 registry 远端检查。
