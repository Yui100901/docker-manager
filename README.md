...
- Registry 同步策略: `dm registry sync` 可按 YAML 策略读取源仓库 tag、应用 include/exclude 规则，并生成 dry-run 同步计划。
...
| `dm registry sync` / `dm report registry-sync` | 按策略生成 registry tag 同步 dry-run 计划，支持多目标和通配符 tag 规则 |
